"""Tests for ProjectRecommendationStatisticsBuilder.
...
class ProjectRecommendationStatisticsTests(unittest.TestCase):
    """Test suite validating project stats builder fields."""

    def test_statistics_fields(self) -> None:
        """Verify built stats dictionary contains timing, workload, success, and count metrics."""
        stats = ProjectRecommendationStatisticsBuilder.build(
        ...
        self.assertEqual(5, stats["projects_processed"])
        self.assertEqual(1, stats["missing_projects"])
        self.assertEqual(2, stats["partial_matches"])
        self.assertEqual(1, stats["related_gaps"])
