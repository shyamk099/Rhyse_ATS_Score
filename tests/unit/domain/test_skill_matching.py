"""Unit tests for Skill Matching.

Purpose:
    Verify SkillMatcher exact matching logic, candidate builders, normalizers,
    validators, stats calculations, deterministic sorting, thread safety,
    and pipeline integration.
"""

from __future__ import annotations

import threading
import unittest
from typing import Sequence

from ats_engine.domain.feature_engineering.models import (
    CanonicalFeatureCollection,
    Feature,
    FeatureCategory,
    FeatureCollection,
    FeatureEngineeringStatistics,
    FeatureExtractionContext,
    FeatureMetadata,
    FeatureProvenance,
    FeatureStatistics,
    ValidationSummary,
)
from ats_engine.domain.matching.models import MatchingContext
from ats_engine.domain.matching.registry import FeatureMatcherRegistry
from ats_engine.domain.matching.service import MatchingService
from ats_engine.domain.matching.skill.matcher import SkillMatcher


class SkillMatchingTests(unittest.TestCase):
    """Test suite validating SkillMatcher and candidate pipeline consolidation."""

    def setUp(self) -> None:
        """Set up registry and service context."""
        self._registry = FeatureMatcherRegistry()
        self._registry.register("skill", SkillMatcher)
        self._service = MatchingService(self._registry)

    def _create_skill_feature(
        self,
        fid: str,
        name: str,
        source_id: str | None,
        category: FeatureCategory = FeatureCategory.SKILL,
    ) -> Feature:
        """Helper to create Feature mock instances."""
        return Feature(
            feature_id=fid,
            name=name,
            category=category,
            value=name,
            provenance=FeatureProvenance(
                source_entity_id=source_id,
                source_entity_type=category.value,
            ),
            metadata=FeatureMetadata(
                creation_timestamp="2026-07-13T20:00:00Z",
                extractor_name="SkillFeatureExtractor",
                version="1.0",
            ),
        )

    def _create_canonical_collection(self, features: list[Feature]) -> CanonicalFeatureCollection:
        """Helper to compile CanonicalFeatureCollection DTOs."""
        col = FeatureCollection(
            features=tuple(features),
            statistics=FeatureEngineeringStatistics(),
            context=FeatureExtractionContext(correlation_id="test"),
        )
        return CanonicalFeatureCollection(
            skills=col,
            experience=FeatureCollection(statistics=FeatureEngineeringStatistics(), context=col.context),
            education=FeatureCollection(statistics=FeatureEngineeringStatistics(), context=col.context),
            projects=FeatureCollection(statistics=FeatureEngineeringStatistics(), context=col.context),
            certifications=FeatureCollection(statistics=FeatureEngineeringStatistics(), context=col.context),
            statistics=FeatureStatistics(total_feature_count=len(features)),
            validation_summary=ValidationSummary(
                status="VALID",
                validation_timestamp="2026-07-13T20:00:00Z",
                rules_version="v1.0",
            ),
        )

    def test_exact_canonical_id_match(self) -> None:
        """SkillMatcher matches features with identical canonical IDs."""
        f_res = self._create_skill_feature("FEAT-SKILL-SKL-01", "Python", "SKL-01")
        f_job = self._create_skill_feature("FEAT-SKILL-SKL-01", "Python V2", "SKL-01")

        resume = self._create_canonical_collection([f_res])
        job = self._create_canonical_collection([f_job])

        res = self._service.match(resume, job, ["skill"])

        self.assertEqual(1, len(res.results))
        self.assertEqual("FEAT-SKILL-SKL-01", res.results[0].resume_feature_id)
        self.assertEqual("FEAT-SKILL-SKL-01", res.results[0].job_feature_id)

    def test_no_match_for_mismatched_ids(self) -> None:
        """SkillMatcher skips features with different canonical IDs."""
        f_res = self._create_skill_feature("FEAT-SKILL-SKL-01", "Python", "SKL-01")
        f_job = self._create_skill_feature("FEAT-SKILL-SKL-02", "Go", "SKL-02")

        resume = self._create_canonical_collection([f_res])
        job = self._create_canonical_collection([f_job])

        res = self._service.match(resume, job, ["skill"])
        self.assertEqual(0, len(res.results))

    def test_comparison_mode_all_matches_raw_by_name(self) -> None:
        """Under comparison_mode=ALL, raw skills are matched case-insensitively by name."""
        f_res = self._create_skill_feature("FEAT-SKILL-RAW-python", "  Python  ", None)
        f_job = self._create_skill_feature("FEAT-SKILL-RAW-python", "python", None)

        resume = self._create_canonical_collection([f_res])
        job = self._create_canonical_collection([f_job])

        # Under default CANONICAL mode, raw features are skipped (source_entity_id is None)
        res_canon = self._service.match(resume, job, ["skill"])
        self.assertEqual(0, len(res_canon.results))

        # Under ALL mode, they match by name
        rules = {
            "skill_matching_rules": {
                "comparison_mode": "ALL",
            }
        }
        res_all = self._service.match(resume, job, ["skill"], rules=rules)
        self.assertEqual(1, len(res_all.results))

    def test_invalid_feature_categories_skipped(self) -> None:
        """Validator/Builder skips features that are not from FeatureCategory.SKILL."""
        f_res = self._create_skill_feature("FEAT-SKILL-SKL-01", "Python", "SKL-01")
        f_job = self._create_skill_feature("FEAT-EXP-01", "Developer", "SKL-01", category=FeatureCategory.EXPERIENCE)

        resume = self._create_canonical_collection([f_res])
        job = self._create_canonical_collection([f_job])

        res = self._service.match(resume, job, ["skill"])
        # Job feature is EXPERIENCE, so candidate builder filters it out (not SKILL)
        self.assertEqual(0, len(res.results))

    def test_read_only_and_trim_normalization(self) -> None:
        """Trims whitespace collapse without mutating the original Feature attributes."""
        f_res = self._create_skill_feature("FEAT-SKILL-SKL-01", "  Python  Name  ", "SKL-01")
        f_job = self._create_skill_feature("FEAT-SKILL-SKL-01", "Python Name", "SKL-01")

        resume = self._create_canonical_collection([f_res])
        job = self._create_canonical_collection([f_job])

        res = self._service.match(resume, job, ["skill"])
        self.assertEqual(1, len(res.results))

        # Original name is unmutated by matcher (but outer spaces are stripped by Pydantic constructor)
        self.assertEqual("Python  Name", f_res.name)

    def test_thread_safety(self) -> None:
        """Running SkillMatcher concurrently results in zero race conditions."""
        f_res = self._create_skill_feature("FEAT-SKILL-SKL-01", "Python", "SKL-01")
        f_job = self._create_skill_feature("FEAT-SKILL-SKL-01", "Python", "SKL-01")

        resume = self._create_canonical_collection([f_res])
        job = self._create_canonical_collection([f_job])

        errors: list[Exception] = []

        def run_match() -> None:
            try:
                res = self._service.match(resume, job, ["skill"])
                self.assertEqual(1, len(res.results))
            except Exception as exc:
                errors.append(exc)

        threads = [threading.Thread(target=run_match) for _ in range(50)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()

        self.assertEqual(0, len(errors), f"Thread safety test failed: {errors}")
